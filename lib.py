...
def load_standards_df(standards_dir, bits):
    ...
    # Construct the pandas DataFrame containing unmasked intensities of elements along with their corresponding mineral
...
def get_standards_weights(standards_dir, minerals):
    ...
    if (standards_dir / 'standards.yaml').exists():
        custom = yaml.safe_load((standards_dir / 'standards.yaml').open('r'))
...
def calculate_element_characteristics(df, elements):
    ...
    for col in elements:
        ...
        # Should fit_intercept be True? The assumption is that it passes through the origin
...
def get_standards_characteristics(standards_dir, bits=32, manual_elements=True):
    ...
    if manual_elements and (standards_dir / 'elements.yaml').exists():
        elements.update(yaml.safe_load((standards_dir / 'elements.yaml').open('r')))

    return elements

def load_target_minerals(target_path):
    return yaml.safe_load(target_path.open('r'))
